...
void	create_cam_default(t_oneweek *one)
{
	...
	cam.horizontal = v_mul_double(cam.u, set.focus_dist * set.aspect_ratio \
	* 2.0f * tanf(degrees_to_radians_float(set.vfov) / 2));
	cam.vertical = v_mul_double(cam.v, set.focus_dist * 2.0 \
	* tanf(degrees_to_radians_float(set.vfov) / 2));
	cam.lower_left_corner = v_minus(v_minus(v_minus(cam.origin, \
	v_mul_double(cam.horizontal, 0.5)), v_mul_double(cam.vertical, 0.5)), \
	v_mul_double(cam.w, set.focus_dist));
	cam.lens_radius = 0;
	cam.time0 = set.time0;
	cam.time1 = set.time1;
	one->cam = cam;
